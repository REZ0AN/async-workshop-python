...
def create_tasks(session, gh_pat, repo_names):
    tasks = []
    headers = {
        "Accept": "application/vnd.github+json",
        "Authorization": f"Bearer {gh_pat}",
...
async def get_lang_in_repositories(gh_pat, repo_names):
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            tasks = create_tasks(session, gh_pat, repo_names)
            responses = await asyncio.gather(*tasks, return_exceptions=True)
            for repo_name, response in tqdm(zip(repo_names, responses), desc="Fetching languages", unit="repo", total=len(repo_names)):
                if isinstance(response, Exception):
                    error_msg = f'Error occurred while fetching language data for {repo_name}: {response}'
                    print(error_msg)
                    logging.error(error_msg)
                    continue
                # Check if response is successful
                if response.status != 200:
                    error_msg = f"API request failed with status {response.status} for {repo_name}: {await response.text()}"
                    print(error_msg)
                    logging.error(error_msg)
                    continue
                ...
                if not isinstance(results, dict):
                    error_msg = f"Unexpected API response format for {repo_name}: {results}"
                    print(error_msg)
                    logging.error(error_msg)
                    continue
...
async def main():
    ...
    df.to_csv(f'{c_dir}/listing_languages_3.csv', index=False)
    end = time.time()
